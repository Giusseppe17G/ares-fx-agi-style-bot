...
Implemented legacy drawdown quarantine for BALANCED_STABLE_MICRO. Added `paper-legacy-drawdown-audit`, separated legacy/unscaled historical halts from active scaled drawdown events, and propagated legacy drawdown status into daily risk, paper risk status, forward evidence and operator dashboard reports. Only `ACTIVE_SCALED_CURRENT_EVENT` can trigger a new active paper halt after valid clearance and daily risk ledgers.
...
Added offline research candidate ranking with `research-candidate-ranking`. The module reads existing SQLite/log/report evidence, extracts symbol and strategy features, classifies candidates, and writes reports under `data/reports/research_candidate_ranking`. Tests cover CLI registration, minimal data, insufficient data, high rejection rate, SQLite non-mutation, and safety flags.
...
Added invalid open paper trade recovery. New modes: `invalid-open-paper-trade-audit` and protected `paper-close-invalid-open-trade`. The audit detects zero risk distance, missing entry/SL, invalid direction and affected trade ids. The close command requires `--trade-id`, `--confirm-paper-only true`, and `--reason`, closes only invalid open paper trades in SQLite, writes a ledger/event report, and clears the paper-state config error only after no invalid open paper trades remain.
...
Added CONFIG_ERROR root-cause resolution. New modes: `config-error-root-cause-audit` and `config-error-fix-plan`. The audit distinguishes missing paths, invalid schemas, profile mismatch, unaccepted gates/ledgers, bad CLI/config evidence, and internal forward-shadow config exceptions. It also detects open paper trades with invalid risk distance without closing or modifying them.
...
Aligned telemetry quarantine with forward acceptance. `quarantine-telemetry-issues` now reports previously quarantined, newly quarantined, and remaining unreviewed historical timestamp issues, writes `telemetry_status_summary.json`, and keeps the quarantine ledger idempotent. `telemetry-status` and `telemetry-acceptance-policy` now expose the same policy counts and write compact JSON reports.
...
Added telemetry drift prevention so forward evidence does not reintroduce unreviewed historical timestamp blockers from old heartbeats, old ML prediction events, redacted legacy timestamps, or invalid timestamp examples generated by reports. Added `telemetry-drift-audit` and report outputs `telemetry_drift_summary.json`, `telemetry_drift_events.csv`, and `telemetry_drift_report.html`.
...
Aligned `forward-acceptance` with legacy drawdown quarantine. Acceptance now uses a consolidated drawdown policy and reports `acceptance_drawdown_blocking`, `acceptance_blocking_reason`, `legacy_drawdown_status`, `active_scaled_drawdown_count`, `drawdown_basis`, and related paper risk fields. Added `acceptance-drawdown-policy-audit`.
...
Added paper state recovery for `PAPER_STATE_ERROR`, `CONFIG_ERROR`, and open paper trade review. New modes: `paper-state-recovery-audit`, `paper-state-recovery-plan`, and protected `paper-close-stale-open-trade`. Forward acceptance now blocks when recovery is required, but allows valid open paper trades to remain open when config is healthy.
