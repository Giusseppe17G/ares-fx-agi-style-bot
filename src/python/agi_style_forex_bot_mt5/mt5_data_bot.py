...
class MT5DataOnlyBot:
    ...
    def run(self) -> MT5DataOnlySummary:
        ...
        for symbol in self.symbols:
            ...
            try:
                accepted = self._process_symbol(symbol, account, counters)
                if not accepted:
    ...
    def _process_symbol(
        ...
        assert self.connector is not None
        check, snapshot = self.connector.ensure_symbol_snapshot(symbol)
        if not check.accepted or snapshot is None:
            self._symbol_rejected(symbol, check.code, check.reason, check.payload)
            return False
        self._audit(
            severity=Severity.INFO,
            module="mt5",
        ...
        bars_by_tf = self._read_timeframes(symbol, snapshot)
        ...
        self._audit(
            ...
            correlation_id=f"{self.run_id}:{symbol}:signal",
            symbol=symbol,
            payload={
    ...
    def _read_timeframes(
        ...
        for timeframe in TIMEFRAMES:
            ...
            raw = self.connector.mt5.copy_rates_from_pos(symbol, mt5_timeframe, 0, self.bars)
            ...
            try:
                ...
                self._audit(
                    ...
                    correlation_id=f"{self.run_id}:{symbol}:{timeframe}",
                    symbol=symbol,
                    payload={"timeframe": timeframe, "bars": len(frame)},
                )
            except Exception as exc:
                self._symbol_rejected(
                    symbol,
    ...
    def _symbol_rejected(
        ...
        self._audit(
            ...
            correlation_id=f"{self.run_id}:{symbol}",
            symbol=symbol,
            payload={"reject_code": reject_code, "reject_reason": reject_reason, **dict(payload or {})},
            notify=True,
        )
...
def summary_to_json(summary: MT5DataOnlySummary) -> str:
